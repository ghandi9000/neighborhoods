...
# Note: if wanting to use BA or DBH - search and replace
# "bvs","bv","BV","BV98",and "BVGROWTH" # will need to be replaced with other
# growth column
...
# Make target dataset with trees from three periods, but no repeats, and
# annualize the BAGROWTH
plots <- c(4:27)
...
ppnew$BAGROWTH <-
  ifelse(!is.na(ppnew$BAGROWTH1),ppnew$BAGROWTH1,
    ifelse(!is.na(ppnew$BAGROWTH2),ppnew$BAGROWTH2,ppnew$BAGROWTH3))
ppnew <- ppnew[ppnew$BAGROWTH >= 0,]
ppnew <- ppnew[-c(104:106)]
# define the target tree dataset
targets <- subset(ppnew,BQUADX > 0 & BQUADX < 11 & BQUADY > 0 & BQUADY < 11) 

# create the neighbors dataset, with plots numbered as in targets, but including
# all neighbors alive at the beginning of the sampling period
...
# define the neighbor trees dataset (the whole dataset)
neighbors <- rbind(neigh1,neigh2,neigh3) 
...
for (i in 1:nrow(targets)) {
  ...
  if (nrow(neighbors.for.tree) > 0) {
    ...
    # Use whatever variable name you have for BA and species codes
    bas[i,1:nrow(neighbors.for.tree)] <- neighbors.for.tree$PRIORBA
...
size.logistic.nci.lambda.model <- function(
  PG, sizeX0, sizeXb, alpha, beta, lambda, C, D) {
  size.effect <- exp(-0.5*(log(targets$PRIORBA/sizeX0)/sizeXb)^2)
  # lambda.vals <-
  # ifelse(neigh.lambda.indexes==0,0,lambda[neigh.lambda.indexes])
...
### Different Param for all species
...
neigh.lambda.indexes <- apply(species, c(1,2), 
  function(x) {
    # create sppcode for nonspecies
    if (x %in% top.neighbors.sppcodes) x else 18
  })
...
### Different Param for inter and intra specific comp
...
### Params for ABBA, PIRU, BECO, SOAM, BEAL, ACSP, rest in OTHERS
...
### Dataset setup ###############                                  
## Dataset of target trees - alive, right species, right size, within max radius
